...
> **這是「每週電子報」的常態格式。** 首封 intro 信(自我介紹+送見面禮、value-first 暖關係,3000+ 喵喵會員一起寄)是另一種、一次性的起手式,設計另談——策略見記憶 `painpoint-newsletter-email-strategy`。從第二週起,每週一封,照下面三鐵則+流程做。

**🎯 每週電子報・三條設計鐵則(2026-06-29 山姆拍板,凌駕底下所有格式細節):**
1. **忠於原文、只濃縮不另寫(=最高鐵則「不能文不對題」):** 信裡丟出的每一個具體鉤子——具名小人物 beat、那個【打比方】、書裡的數字、結尾「這禮拜小作業」——都得是該 `/lens` 拆書**真的有**的東西;讀者點進去要看到摘要承諾的同一塊,不可撲空。是「濃縮+設懸念」,不是另寫一篇,也**不整段照抄原句**。
2. **留懸念當點擊引擎(不破梗):** 把全文最有勁的兩三個點寫成「鉤子」但**故意不給答案**(例《鉤癮效應》那封:「他漏了什麼」「那塊地基是什麼」「小作業是什麼」全留在全文不破梗)——答案在 painpoint,讀者非點進去不可。摘要的工作是「讓人想知道」,不是「替他讀完」。
3. **書放最後、一句帶過(電子報=給價值,不是賣書):** 通篇主角是這篇拆書的價值;《痛點》電子書只在結尾輕輕一句、配 `https://painpoint.tw/book`、可再補「不急」。過度賣書會像 `/lens` 過度 CTA 一樣毀掉信任——這條凌駕導購。
