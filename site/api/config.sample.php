...
return [
  ...
  // /claim 讀者好禮「書中通關密語」閘門:擋掉沒買書的人來領、保護「已購買-痛點」標籤純度。
  //   - claim_question:顯示在表單上的問題(非機密,可寫這裡或留空用預設)。
  //   - claim_passphrase:正確答案(機密;選「書裡印著、但站上/Google 查不到」的內容當答案)。
  //   兩個都填好,/claim 才會出現密語欄並啟用驗證;任一留空 = 閘門關閉(維持開放領取)。
  //   比對時會自動去頭尾空白、忽略大小寫、移除空格,所以買家不必逐字一模一樣。
  'claim_question'   => '',   // 例:書裡第三部那張「動手卡」最上面那三個字是?
  'claim_passphrase' => '',   // 例:越賭越大  (答案;留空=不啟用閘門)
