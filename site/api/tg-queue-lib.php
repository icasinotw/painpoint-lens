...
const TG_STREAK_THRESHOLD = 4;   // 「不分種類」的連續失敗斷路器:連續達此數(任何失敗:品質/push/render/引擎都算,中間沒有一本成功)
                                 // → 暫停整列 + 發醒目警示。為什麼要這道:per-lane 引擎斷路器只擋「額度滿」,但「每本都卡同一個
                                 //   系統性 bug」(例 2026-06-30 .ci/ 卡 rebase)會被歸成 quality → 永不暫停、無限空燒額度。這道是兜底:
                                 //   把「整夜白燒幾十本」變「燒 4 本就暫停叫醒主人」。設成手動式暫停(cron 不自動解),非要 /resume 才續。
...
function _q_load($fh) {
    ...
    return [
        ...
        'fail_streak' => (int)($d['fail_streak'] ?? 0),   // 不分種類的連續失敗數(任何成功就歸零);達 TG_STREAK_THRESHOLD → 斷路器暫停
...
function q_report($status, $kind = '', $book = '', $lane = '') {
    ...
    if ($success) {
        $d['fail_streak'] = 0;   // 有一本成功 → 系統是活的 → 連敗斷路器歸零
