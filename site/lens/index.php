...
// ★固定★頁籤順序:依「痛點」重要性遞減,不隨發新書的篇數變動——與下方「依主題深入讀」
// 卡片同一套 $PAIN_CAT_ORDER 邏輯(見該處註解)。要改順序只改那張表;沒列到的排最後、彼此依篇數。
...
// 沿用上方頁籤那張 $PAIN_CAT_ORDER(同一套痛點重要性邏輯,頁籤與主題卡永遠同序):
//   ① 解客戶的痛(對外、創造價值,痛點核心):創業→創新→行銷→敘事→策略
//   ② 經營組織(把事做成):領導→管理
//   ③ 改變自己(模式 C,對內,離解客戶的痛最遠):行為→生產力→理財  ④ 其他兜底
// 沒列到的(未來新達標主題)自動排最後、彼此依篇數。
