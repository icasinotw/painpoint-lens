...
def card_titles():
    """回傳 {slug: title}(從 _articles.php 用 entry block 穩健解析)。"""
    txt = open(os.path.join(LENS, '_articles.php'), encoding='utf-8').read()
    out = {}
    for b in re.findall(r"\[\s*'slug'.*?\],", txt, re.S):
        ms = re.search(r"'slug'\s*=>\s*'([^']+)'", b)
        mt = re.search(r"'title'\s*=>\s*'(.*?)'\s*,\s*\n", b, re.S)
        if ms and mt:
            out[ms.group(1)] = mt.group(1)
    return out
